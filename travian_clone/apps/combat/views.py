...
class HeroImageView(APIView):
    ...
    def get(self, request):
        ...
        if preview:
            gender = request.query_params.get('gender', hero.gender)
            hair_color = int(request.query_params.get('hair_color', hero.hair_color))
            hair_style = int(request.query_params.get('hair_style', hero.hair_style))
            eye_style = int(request.query_params.get('eye_style', hero.eye_style))
            eyebrow_style = int(request.query_params.get('eyebrow_style', hero.eyebrow_style))
            nose_style = int(request.query_params.get('nose_style', hero.nose_style))
        ...
        color = self.COLOR_MAP.get(hair_color, 'black')
        ...
        eye_idx = (eye_style - 1) % 5
        eye = load(os.path.join(base_path, 'eye', 'eye{}.png'.format(eye_idx)))
        if eye: body.paste(eye, (0, 0), eye)
        eyebrow_count = 10 if gender == chr(70)+chr(69)+chr(77)+chr(65)+chr(76)+chr(69) else 5
        eyebrow_idx = (eyebrow_style - 1) % eyebrow_count
        if gender == 'MALE':
            eyebrow = load(os.path.join(base_path, 'eyebrow', 'eyebrow{}-{}.png'.format(eyebrow_idx, color)))
        else:
            eyebrow = load(os.path.join(base_path, 'eyebrow', 'eyebrow{}.png'.format(eyebrow_idx)))
        if eyebrow: body.paste(eyebrow, (0, 0), eyebrow)
        hair_idx = (hair_style - 1) % 5
        hair = load(os.path.join(base_path, 'hair', 'hair{}-{}.png'.format(hair_idx, color)))
        if hair: body.paste(hair, (0, 0), hair)
        ear_count = 8 if gender == chr(70)+chr(69)+chr(77)+chr(65)+chr(76)+chr(69) else 5
        ear_idx = (ear_style - 1) % ear_count
        ear = load(os.path.join(base_path, 'ear', 'ear{}.png'.format(ear_idx)))
        if ear: body.paste(ear, (0, 0), ear)
        mouth_count = 9 if gender == chr(70)+chr(69)+chr(77)+chr(65)+chr(76)+chr(69) else 4
        mouth_idx = (mouth_style - 1) % mouth_count
        mouth = load(os.path.join(base_path, 'mouth', 'mouth{}.png'.format(mouth_idx)))
        if mouth: body.paste(mouth, (0, 0), mouth)
        nose_idx = (nose_style - 1) % 5
        nose = load(os.path.join(base_path, 'nose', 'nose{}.png'.format(nose_idx)))
        if nose: body.paste(nose, (0, 0), nose)
        face_idx = (head_style - 1) % 5
