...
export default function Stable() {
    ...
    const fetchQueue = useCallback(async () => {
        ...
        try {
            const { data } = await api.get('combat/barracks/queue/', { params: { village_id: activeVillageId, building_type: 'stable' } });
            setQueue(data);
        } catch (error) { console.error(error); }
    }, [activeVillageId]);
    ...
    if (loading) return <PageShell><LoadingState label="در حال بارگذاری اصطبل..." /></PageShell>;

    return (
        <PageShell maxWidth="max-w-4xl">
            <div className="panel">
                <div className="panel-header">
                    <span className="panel-title">🐎 اصطبل</span>
                </div>
                <div className="panel-body">
                    <p className="text-ink-500 text-xs mb-5">
                        در اینجا نیروهای سواره‌نظام را آموزش می‌دهید. پس از اتمام زمان آموزش، نیروها به دهکده اضافه می‌شوند.
                    </p>

                    <div className="space-y-3">
                        {catalog.filter(unit => unit.is_cavalry).map((unit) => {
            ...
            <div className="panel">
                <div className="panel-header"><span className="panel-title">⏳ صف آموزش اصطبل فعلی</span></div>
                <div className="panel-body">
                    {queue.length === 0 ? (
                        <EmptyState icon="🐎" title="در حال حاضر هیچ نیروی سواره‌ای در حال آموزش نیست." />
                    ) : (
