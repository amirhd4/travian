...
const getAssetPath = (building) => {
    if (building.level === 0 && !building.is_upgrading) {
        return '/assets/buildings/empty_slot.png';
    }
    ...
    const nameMap = {
        ...
        'خزانه‌داری': 'treasury',
        'آکادمی': 'academy',
        'عمارت اقامتی': 'residence',
        'تالار شهر': 'town_hall',
        'مخفیگاه': 'hideout',
        'آهنگری': 'blacksmith',
        'کارگاه سنگ‌تراشی': 'stonemason',
        'عمارت قهرمان': 'hero_mansion',
        'آبشخور اسب': 'horse_trough',
        'اداره تجارت': 'trade_office',
        'پادگان بزرگ': 'great_barracks',
        'محل گردهمایی': 'rally_point',
        'دیوار (Wall)': 'wall',
        'شگفتی جهان': 'world_wonder',
    };
...
export default function VillageCenter() {
    ...
    const fetchBuildings = useCallback(async () => {
        ...
        try {
            ...
            console.error("خطا در دریافت اطلاعات مرکز دهکده", error);
        } finally {
            setLoading(false);
        }
    }, [activeVillageId]);
    ...
    useEffect(() => {
        ...
        async function renderScene(app) {
            ...
            try {
                ...
                fallbackBg.rect(0, 0, app.screen.width, app.screen.height).fill({ color: 0x769b50 });
    ...
    return (
        <div className="w-full min-h-screen flex flex-col items-center pt-32 pb-10" style={{ backgroundImage: "url('/assets/bgs/bgVillage-rtl.jpg')", backgroundSize: "cover", backgroundPosition: "center" }}>
            <ResourceBar />
            <Navbar />

            {loading ? (
                <p className="font-bold text-[#3d2b1a] mt-10">در حال بارگذاری مرکز دهکده...</p>
            ) : (
                <div className="flex flex-col items-center w-full max-w-4xl">
                    ...
                    <div className="shadow-2xl border-8 border-[#593d2b] rounded-lg overflow-hidden relative bg-black" ref={pixiContainerRef} style={{ width: '620px', height: '460px', maxWidth: '100%' }} />
                    ...
                    <div className="bg-[#f4ebd0] border-4 border-[#593d2b] rounded-xl shadow-2xl max-w-sm w-full p-6 relative">
                        ...
                        {selectedSlot.is_upgrading ? (
                            ...
                            <div className="bg-green-100 border border-green-500 rounded p-3 text-center mb-4">
                                <p className="text-sm font-bold text-green-800">🏆 این ساختمان به حداکثر سطح رسیده است.</p>
                            </div>
                        ) : (
